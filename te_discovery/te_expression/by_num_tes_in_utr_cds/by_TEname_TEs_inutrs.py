...
[os.remove(f) for f in glob.glob('name/*.pdf')]
...
for TE in dfam:
    if TE['type'] not in ('DNA', 'LINE', 'SINE', 'LTR', 'Retroposon'):
        continue

    te_tpms = get_num_te_in_utr(contains_te, TE=TE['name'])

    if max([len(te_tpms['utr5']), len(te_tpms['cds']), len(te_tpms['utr3'])]) < 50: # Ignore uncommon/empty
        continue

    print(TE['type'], TE['subtype'], TE['name'])
    ...
    shared.boxplots('name/num_tes_pc-{0}-{1}-{2}.pdf'.format(TE['type'], TE['subtype'], TE['name']),
        res_pc, qs=q_pc,
        title='{0}:{1}:{2}'.format(TE['type'], TE['subtype'], TE['name']),
        trim_low_samples=10)
