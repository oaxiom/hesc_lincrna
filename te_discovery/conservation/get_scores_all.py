...
# Sanity checking you are doign the strand correctly:
...
for i in res:
    if i['strand'] == '+':
        x.append(i['phyloP_tes'])
        y.append(i['phyloP_nottes'])
ax.scatter(x, y, s=4, alpha=0.1, c='red', ec='none')
...
for i in res:
    if i['strand'] == '-':
        x.append(i['phyloP_tes'])
        y.append(i['phyloP_nottes'])
ax.scatter(x, y, s=4, alpha=0.1, c='blue', ec='none')

#ax.set_xlim(xlims)
#ax.set_ylim(ylims)
fig.savefig('strand_check.pdf')
